...
export const medicoService = {
  ...
  historialPaciente: async (dni: string): Promise<TurnoResponse[]> => {
    const response = await api.get<any[]>('/api/profesionales/historial-paciente', { params: { dni } });
    return response.data.map((item) => appointmentService.normalizeForStaff(item));
  },
...
export const secretariaService = {
  turnos: async (): Promise<TurnoResponse[]> => {
    const response = await api.get<any[]>('/api/turnos');
    return response.data.map((item) => appointmentService.normalizeForStaff(item));
  },
  buscarPaciente: async (dni: string) => {
    const response = await api.get<any>('/api/secretaria/pacientes/buscar', { params: { dni } });
