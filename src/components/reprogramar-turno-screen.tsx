...
import { ReprogramarTurnoView } from './reprogramar-turno-view';
...
export function ReprogramarTurnoScreen({ eyebrow, backTitle, backPath, navigation }: Props) {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      setNotice({
        ...
        message: readableError(error, 'No se pudo cargar la información del turno.'),
      });
    } finally {
      setLoading(false);
  ...
  const selectDate = (date: string) => {
    ...
    setSelectedSlot(availableSlots.find((slot) => slot.fecha === date) ?? null);
    setShowDates(false);
    setShowTimes(true);
    setNotice(null);
  };
  ...
  const confirm = async () => {
    ...
    if (!turno || !selectedSlot || !turno.profesionalId) {
      setNotice({
        type: 'error',
        title: 'Falta información',
        message: 'Seleccioná una nueva fecha y hora válidas.',
      });
      return;
    }
    ...
    try {
      ...
      setNotice({
        ...
        message: readableError(error, 'El horario no está disponible o faltan datos del turno.'),
      });
    } finally {
  ...
  return (
    <ReprogramarTurnoView
      ...
      showTimes={showTimes}
      notice={notice}
      updatedTurno={updatedTurno}
      onSelectDate={selectDate}
      onSelectSlot={selectSlot}
      onToggleDates={() => {
        setShowDates((value) => !value);
        setShowTimes(false);
      }}
      onToggleTimes={() => {
