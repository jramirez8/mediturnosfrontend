...
export function AdminNotice({ type = 'info', title, message, onRetry }: { type?: 'info' | 'success' | 'danger' | 'warning'; title: string; message?: string; onRetry?: () => void }) {
  ...
  return (
    <MtCard style={{ borderColor: color, marginBottom: 14 }}>
      ...
      {onRetry ? <MtButton title="Reintentar" onPress={onRetry} style={{ marginTop: 12 }} /> : null}
    </MtCard>
  );
}
...
export function AdminKV({ label, value }: { label: string; value?: string | number | null }) {
  ...
  return (
    <View style={{ marginTop: 5 }}>
      <Text style={{ color: theme.colors.soft, fontSize: 11, fontWeight: '900', textTransform: 'uppercase' }}>{label}</Text>
      <Text style={{ color: theme.colors.muted, fontWeight: '700', marginTop: 2 }}>{value || 'No informado'}</Text>
    </View>
  );
}
...
export function AdminTitle({ title, subtitle }: { title: string; subtitle?: string }) {
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      <Text style={{ color: theme.colors.ink, fontWeight: '900', fontSize: 18 }}>{title}</Text>
      {subtitle ? <Text style={{ color: theme.colors.muted, marginTop: 4, lineHeight: 19 }}>{subtitle}</Text> : null}
    </View>
  );
}
