...
export function MtButton({
  ...
  return (
    <Pressable onPress={onPress} disabled={disabled || loading} style={({ pressed }) => [buttonStyle, pressed && { transform: [{ scale: 0.99 }] }]}>
      {loading ? <ActivityIndicator color="white" /> : <Text style={textStyle}>{title}</Text>}
    </Pressable>
  );
}
...
export function MtPill({ label, tone = 'primary', selected = false, onPress }: { label: string; tone?: 'primary' | 'success' | 'warning' | 'danger' | 'muted'; selected?: boolean; onPress?: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} disabled={!onPress} style={[styles.pill, selected && { backgroundColor: toneColor, borderColor: toneColor }]}>
      <Text style={[styles.pillText, { color: selected ? '#FFFFFF' : toneColor }]}>{label}</Text>
    </Pressable>
  );
}
...
export function MtBottomNav({ active }: { active: 'home' | 'perfil' | 'turnos' | 'historia' | 'solicitar' | 'profesionales' }) {
  ...
  const item = (key: typeof active, label: string, path: string) => (
    <Pressable style={styles.navItem} onPress={() => router.replace(path as any)}>
      <Text style={[styles.navEmoji, active === key && { color: theme.colors.primary }]}>
        {key === 'home' ? '⌂' : key === 'perfil' ? '◉' : key === 'turnos' ? '□' : key === 'historia' ? '▤' : '+'}
      </Text>
      <Text style={[styles.navText, active === key && { color: theme.colors.primary, fontWeight: '800' }]}>{label}</Text>
    </Pressable>
  );

  return (
    <View style={styles.navBar}>
      {item('home', 'Inicio', '/paciente')}
      {item('perfil', 'Perfil', '/paciente/perfil')}
      <Pressable style={styles.navFab} onPress={() => router.push('/paciente/solicitar')}>
        <Text style={styles.navFabText}>+</Text>
      </Pressable>
      {item('turnos', 'Turnos', '/paciente/turnos')}
      {item('historia', 'Historia', '/paciente/historia')}
    </View>
  );
}
...
function createStyles(theme: MediturnosTheme) {
  return StyleSheet.create({
    ...
    navEmoji: { fontSize: 20, color: theme.colors.soft, fontWeight: '900' },
    navText: { color: theme.colors.soft, fontSize: 10, fontWeight: '700' },
    navFab: { width: 58, height: 58, marginTop: -26, borderRadius: 29, backgroundColor: theme.colors.primary, alignItems: 'center', justifyContent: 'center', ...theme.shadow },
